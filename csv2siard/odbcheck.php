...
while ($row = odbc_fetch_array ($recordset)) {
	if ($recordcount == 0) {
		...
		log_echo(strtoupper(implode($prg_option['DELIMITED'], $header))."\n");
	}
	log_echo(ansi2ascii(implode($prg_option['DELIMITED'], $row))."\n");
	$recordcount++;
}
